...
for var in lvar:
  for plev in lplev:
    for mon in lmon:
      ...
      ctlname   = idir + "/%s_%s.monthly.ctl"%(stype,var)
      ...
      stime     = "%02d"%(mon)
      #----- Names ------------
      gribname  = idir + "/%s_%s.clim8110.mon%s"%(stype, var, stime)
    ...
    for mon in lmon:
      ...
      stime = "%02d"%(mon)
      iname = odir + "/%s.%s.%04dhPa.%s.%s"%(stype, var, plev, stime, res)
      a2mon = fromfile(iname, float32).reshape(ny_org, nx_org)
      a2mon = a2mon
      a2ann = a2ann + a2mon * days

    a2ann = a2ann / 365.
    oname = odir + "/%s.%s.%04dhPa.00.%s"%(stype, var, plev, res)
    a2ann.tofile(oname) 
    print oname
